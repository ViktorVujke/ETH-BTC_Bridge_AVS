...
type AvsWriterer interface {
	...
	SendNewTaskCheckTransaction(
		ctx context.Context,
		btcTxHash string,
		signedMessage string,
		mintTo gethcommon.Address,
...
func (w *AvsWriter) SendNewTaskCheckTransaction(ctx context.Context, btcTxHash string, signedMessage string, mintTo gethcommon.Address, quorumThresholdPercentage sdktypes.QuorumThresholdPercentage, quorumNumbers sdktypes.QuorumNums) (cstaskmanager.IIncredibleSquaringTaskManagerTask, uint32, error) {
	...
	tx, err := w.AvsContractBindings.TaskManager.CreateNewTask(txOpts, btcTxHash, signedMessage, mintTo, big.NewInt(0), "", false, uint32(quorumThresholdPercentage), quorumNumbers.UnderlyingType())
	if err != nil {
	...
	newTaskCreatedEvent, err := w.AvsContractBindings.TaskManager.ContractIncredibleSquaringTaskManagerFilterer.ParseNewMintTaskCreated(*receipt.Logs[0])
	if err != nil {
...
func (w *AvsWriter) SendNewTaskBurnBtc(ctx context.Context, btcDestinationAddress string, burnAmount *big.Int, quorumThresholdPercentage sdktypes.QuorumThresholdPercentage, quorumNumbers sdktypes.QuorumNums) (cstaskmanager.IIncredibleSquaringTaskManagerTask, uint32, error) {
	txOpts, err := w.TxMgr.GetNoSendTxOpts()
	...
	tx, err := w.AvsContractBindings.TaskManager.CreateNewTask(txOpts, "", "", gethcommon.Address{}, burnAmount, btcDestinationAddress, true, uint32(quorumThresholdPercentage), quorumNumbers.UnderlyingType())
	if err != nil {
		w.logger.Errorf("Error assembling CreateNewTask tx")
		return cstaskmanager.IIncredibleSquaringTaskManagerTask{}, 0, err
	}
	...
	newTaskCreatedEvent, err := w.AvsContractBindings.TaskManager.ContractIncredibleSquaringTaskManagerFilterer.ParseNewBurnTaskCreated(*receipt.Logs[0])
	if err != nil {
		w.logger.Error("Aggregator failed to parse new task created event", "err", err)
		return cstaskmanager.IIncredibleSquaringTaskManagerTask{}, 0, err
	}
...
// returns the tx receipt, as well as the task index (which it gets from parsing the tx receipt logs)
// sends new task to confirm the BTC transaction
